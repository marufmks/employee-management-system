  ...
  return (
    <Card>
      <CardHeader>
        <h2>
          {isEditing
            ? __("Edit Employee", "employee-management-system")
      ...
      <CardBody>
        ...
        >
          <Panel>
            <PanelBody
              title={__("Basic Information", "employee-management-system")}
              initialOpen={true}
            >
              <SelectControl
              ...
              <div className="form-row">
                <TextControl
                  ...
                  onChange={(value) =>
                    handleInputChange("starting_salary", value)
                  }
              ...
              <div className="form-row">
                ...
                <SelectControl
                  ...
                  options={[
                    {
                      label: __("Single", "employee-management-system"),
                      value: "single",
                    },
                    {
                      label: __("Married", "employee-management-system"),
                      value: "married",
                    },
                  ]}
                  onChange={(value) =>
              ...
              <SelectControl
                ...
                options={[
                  {
                    label: __("Active", "employee-management-system"),
                    value: "active",
                  },
                  {
                    label: __("Inactive", "employee-management-system"),
                    value: "inactive",
                  },
                  {
                    label: __("Blocked", "employee-management-system"),
                    value: "blocked",
                  },
                ]}
          ...
          <div className="panel-grid">
            ...
            <Panel>
              <PanelBody
                title={__("Address Information", "employee-management-system")}
                initialOpen={true}
              >
                <TextareaControl
            ...
            <Panel>
              <PanelBody
                title={__("Emergency Contact", "employee-management-system")}
                initialOpen={true}
              >
                <TextControl
...
const EmployeeList = ({ employees, onEdit, onDelete }) => {
  ...
  const downloadEmployeeCSV = async (employeeId) => {
    try {
      ...
      const csvRows = [
        [
          'Name', 'Email', 'Department', 'Designation', 'Join Date', 'Leave Date',
          'Starting Salary', 'Current Salary', 'Phone', 'Street Address', 'City',
          'State', 'Postal Code', 'Country', 'Emergency Contact Name',
          'Emergency Contact Phone', 'Emergency Contact Relation', 'Marital Status',
          'Status'
        ], // Headers
        ...response.map(row => [
          row.name || 'N/A',
          ...
          row.join_date || 'N/A',
          row.leave_date || 'N/A',
          row.starting_salary || 'N/A',
          row.current_salary || 'N/A',
          row.phone || 'N/A',
          `"${((row.street_address || 'N/A').replace(/"/g, '""'))}"`,
          row.city || 'N/A',
          row.state || 'N/A',
          row.postal_code || 'N/A',
          row.country || 'N/A',
          row.emergency_contact_name || 'N/A',
          row.emergency_contact_phone || 'N/A',
          row.emergency_contact_relation || 'N/A',
      ...
      toast.success(__('Employee details downloaded successfully', 'employee-management-system'));
    } catch (error) {
      toast.error(error.message || __('Failed to download employee details', 'employee-management-system'));
    }
  };
                    ...
                    case "active":
                      return (
                        <span className="status-active">
                          {__("Active", "employee-management-system")}
                        </span>
                      );
                    case "inactive":
                      return (
                        <span className="status-inactive">
                          {__("Inactive", "employee-management-system")}
                        </span>
                      );
                    case "blocked":
                      return (
                        <span className="status-blocked">
                          {__("Blocked", "employee-management-system")}
                        </span>
                      );
                    default:
                      return (
                        <span className="status-unknown">
                          {__("Unknown", "employee-management-system")}
                        </span>
                      );
                  }
                })()}
...
const Employees = () => {
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      toast.error(
        err.message ||
          __("Failed to save employee.", "employee-management-system")
      );
      return false;
    }
  };
  ...
  const handleEdit = (employee) => {
    const formattedEmployee = {
      ...
      join_date: employee.join_date
        ? formatDateForInput(employee.join_date)
        : "",
      leave_date: employee.leave_date
        ? formatDateForInput(employee.leave_date)
        : "",
    };
  ...
  if (error)
    return (
      <div>
        {__("Error:", "employee-management-system")} {error}
      </div>
    );
